...
if %ERRORLEVEL% neq 0 (
    ...
    echo [%date% %time%] Error: Failed to activate Conda base environment >> "%WORKSPACE_DIR%startup_log.txt"
    pause
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    echo Error: Python not found in Conda base environment
    echo [%date% %time%] Error: Python not found in Conda base environment >> "%WORKSPACE_DIR%startup_log.txt"
    pause
    exit /b 1
)
...
start cmd /k "conda activate base && cd /d "%WORKSPACE_DIR%" && echo Redis CLI starting... && "%WORKSPACE_DIR%849\redis\redis-server.exe""
...
start cmd /k "conda activate base && cd /d "%WORKSPACE_DIR%" && echo PAD service starting... && "%WORKSPACE_DIR%849\pad\main.exe""
